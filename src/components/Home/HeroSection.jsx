...
const HeroSection = () => {
  ...
  // (2) Defer heavy FX until after first paint
  ...
  // (2) Turn on FX after first paint (and small delay)
  useEffect(() => {
  ...
  // Underwater caustics effect (runs only when fxOn)
  useEffect(() => {
    if (!fxOn) return;
  ...
  // (1) Mouse-reactive glow without React re-renders
  useEffect(() => {
    let raf = 0;
  ...
  return (
    <section ref={containerRef} className="relative h-screen w-full overflow-hidden bg-black">
      {/* Caustics canvas (only after fxOn) */}
      {fxOn && (
        <canvas
          ref={canvasRef}
      ...
      {/* Mouse-reactive caustic glow (no re-renders) */}
      <div
        ref={glowRef}
      ...
      {/* Floating bubbles/particles (defer until fxOn) */}
      {fxOn && (
      ...
      {/* Animated underwater light rays (defer until fxOn) */}
      {fxOn && (
      ...
      >
        {/* Orbiting bubbles around center (defer until fxOn) */}
        {fxOn && (
          <div className="absolute inset-0 pointer-events-none">
            {[...Array(4)].map((_, i) => (
              <motion.div
                ...
                style={{
                  ...
                  height: "12px",
                  background:
                    "radial-gradient(circle, rgba(34, 211, 238, 0.8) 0%, rgba(34, 211, 238, 0.2) 70%)",
                  boxShadow:
                    "0 0 30px rgba(34, 211, 238, 0.8), inset 0 0 15px rgba(255, 255, 255, 0.5)",
                }}
                animate={{
        ...
        >
          ...
          <motion.span
            ...
            style={{
              textShadow:
                "0 0 50px rgba(34, 211, 238, 0.6), 0 0 100px rgba(34, 211, 238, 0.3), 0 4px 20px rgba(0,0,0,0.5)",
          ...
          <motion.span
            ...
            style={{
              filter:
                "drop-shadow(0 0 60px rgba(59,130,246,0.8)) drop-shadow(0 0 30px rgba(6,182,212,0.6))",
            }}
